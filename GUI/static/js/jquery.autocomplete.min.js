...
function highlight_nodes() {
	if (global_c.length !== 0) {
		var c = global_c;
		search_list_suggestions = [];
		for (var i = 0; i < c.suggestions.length; i++) {
			search_list_suggestions.push(data.nodes.get(c.suggestions[i].value).id)
		}
		network.selectNodes(search_list_suggestions,true);
		global_c = [];
	}
	else {
		displayNotification("warning","Please enter a search");
	}
}
...
this.hide():this.getSuggestions(a)},getQuery:function(a){var b=this.options.delimiter;if(!b)return e.trim(a);a=a.split(b);return e.trim(a[a.length-1])},getSuggestionsLocal:function(a){var b=a.toLowerCase(),c=this.options.lookupFilter;return{suggestions:e.grep(this.options.lookup,function(d){return c(d,a,b)})}},
...
getSuggestions : function(a) {
	...
	// Make c globally accessible
	global_c = c;

	(b = c.isLocal?c.getSuggestionsLocal(a) : c.cachedResponse[a]) && e.isArray(b.suggestions)?(c.suggestions = b.suggestions,c.suggest()) : c.isBadQuery(a) || (d.params[d.paramName] = a,
